...
void YoloObjectDetector::init() {
  ...
  nodeHandle_.param("subscribers/camera_reading/topic", cameraTopicName, std::string("/usb_cam/image_raw")); //"/camera/image_raw"
  ...
  numberPublisher_= 
      nodeHandle_.advertise<std_msgs::Int16>("/darknet_result", 1, true);
...
void YoloObjectDetector::yolo() {
  ...
  if (!demoPrefix_ && viewImage_) {
    cv::namedWindow("YOLO", cv::WINDOW_NORMAL); //create window
    if (fullScreen_) {
